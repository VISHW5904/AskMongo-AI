...
class EnhancedMongoDBBot:
    ...
    that can be executed by PyMongo's `collection.find()`, `collection.aggregate()`, `collection.distinct()`, or `collection.count_documents()` methods.
    ...
    7. For distinct values, use distinct("fieldName", {filter})
    8. For counts, use count_documents({filter})
    9. Date Handling: If a specific date like "01/01/2025" is mentioned, filter for the entire day. For date ranges like "between 01/01/2025 and 03/01/2025", use $gte for the start date and $lt for the day after the end date. Convert all date strings to ISODate format, e.g., ISODate("YYYY-MM-DDTHH:MM:SSZ").
    10. Comparisons: For "greater than", use $gt. For "less than", use $lt.
    11. Top/Bottom N: Use $sort and $limit in an aggregation pipeline. For "top" use $sort: -1, for "least" or "lowest" use $sort: 1. Ensure the field for sorting is correct (e.g., "$qty" for quantity, or a summed field like "$totalQuantity" if grouped).
    12. Averages: Use $avg in a $group stage.
    13. Minimum/Maximum: Use $min/$max in a $group stage, or $sort + $limit for records.
    14. Counts: For specific counts, use count_documents({filter}). For grouped counts, use $sum: 1 in $group.
    15. Handle numbers as numbers in queries, not strings, unless they are codes.
    16. Ensure all MongoDB operators (like $match, $group, $gte, $sum, $avg, $min, $max, $sort, $limit) and field names (like memberCode, qty) are enclosed in double "quotes".
    17. If the user asks for results by month, group by the month extracted from the date field using {"month": {"$month": "$dateTimeOfCollection"}} in the aggregation _id.
    18. If the user asks for specific months (e.g., November and December), include a filter in the $match stage using $expr and $in with $month, e.g., {"$expr": {"$and": [ ... ]}}. Do not use $and in $match for month filtering; always use $expr and $in with $month and $year for month-based filtering.
    19. For trend questions (trend of X per month/year), group by month/year and use $avg/$sum as needed, and $sort by time.
    20. For compare questions (compare X for member A and member B), use $match with $in for codes, group by code and time if needed.
    21. For top/least by field, use $group, $sort, and $limit.
    22. For lowest/highest value on a date, use $match for the date, $sort, and $limit:1.
    23. For unique values, use distinct.
    ...
    find({"fat": {"$gt": 4.1}})
    find({"memberCode": "0010000019930016", "fat": {"$lt": 4.1}, "dateTimeOfCollection": {"$gte": ISODate("2025-01-01T00:00:00Z"), "$lt": ISODate("2025-02-01T00:00:00Z")}})
    find({"dcsCode": "001000001993", "fat": {"$gt": 4.1}, "dateTimeOfCollection": {"$gte": ISODate("2025-01-01T00:00:00Z"), "$lt": ISODate("2025-02-01T00:00:00Z")}})
    ...
    aggregate([{"$match": {"$expr": {"$and": [
        {"$in": ["$memberCode", ["0010000019930016", "0010000004790107"]]},
        {"$in": [{"$month": "$dateTimeOfCollection"}, [11, 12]]},
        {"$eq": [{"$year": "$dateTimeOfCollection"}, 2024]}
    ]}}}, {"$group": {"_id": {"memberCode": "$memberCode", "month": {"$month": "$dateTimeOfCollection"}}, "averageQty": {"$avg": "$qty"}}}, {"$sort": {"_id.memberCode": 1, "_id.month": 1}}])
    ...
    aggregate([{"$match": {"$expr": {"$and": [
        ...
        {"$in": [{"$month": "$dateTimeOfCollection"}, [11, 12]]},
        {"$eq": [{"$year": "$dateTimeOfCollection"}, 2024]}
    ]}}}, {"$group": {"_id": {"memberCode": "$memberCode", "month": {"$month": "$dateTimeOfCollection"}}, "avgQty": {"$avg": "$qty"}, "avgSnf": {"$avg": "$snf"}, "avgFat": {"$avg": "$fat"}}}, {"$sort": {"_id.memberCode": 1, "_id.month": 1}}])
    aggregate([{"$match": {"memberCode": {"$in": ["0010000008650047", "760540500086"]}}}, {"$group": {"_id": "$memberCode", "avgQty": {"$avg": "$qty"}, "avgFat": {"$avg": "$fat"}, "avgSnf": {"$avg": "$snf"}}}])
    aggregate([{"$match": {"$expr": {"$and": [
        {"$eq": ["$memberCode", "0010000008650047"]},
        {"$in": [{"$month": "$dateTimeOfCollection"}, [11, 12]]}
    ]}}}, {"$group": {"_id": {"month": {"$month": "$dateTimeOfCollection"}}, "avgQty": {"$avg": "$qty"}, "avgFat": {"$avg": "$fat"}, "avgSnf": {"$avg": "$snf"}}}, {"$sort": {"_id.month": 1}}])
    aggregate([{"$match": {"$expr": {"$and": [
        {"$in": ["$dcsCode", ["001000001993", "001000002000"]]},
        {"$eq": [{"$year": "$dateTimeOfCollection"}, 2024]}
    ]}}}, {"$group": {"_id": {"dcsCode": "$dcsCode", "year": {"$year": "$dateTimeOfCollection"}}, "totalQty": {"$sum": "$qty"}}}, {"$sort": {"_id.dcsCode": 1, "_id.year": 1}}])
    count_documents({"memberCode": "730110400002"})
    count_documents({"dcsCode": "001000001993", "dateTimeOfCollection": {"$gte": ISODate("2025-01-01T00:00:00Z"), "$lt": ISODate("2025-03-02T00:00:00Z")}})
    aggregate([{"$match": {}}, {"$group": {"_id": None, "avgQty": {"$avg": "$qty"}}}])
    ...
    def _safe_eval(self, query_string):
        ...
        # --- Fix for malformed empty dicts from LLM output ---
        ...
        # --- Fix for missing closing brackets in $gte/$lt/$lte/$gt/$eq/$ne/$in expressions ---
        # Add missing ] for $op: [ ...
        query_string = re.sub(r'(\{"\$(gte|lt|lte|gt|eq|ne|in)": \[[^\]]+)(?=,|\})', r'\1]', query_string)
        # Add missing } for $op: [ ... ]
        query_string = re.sub(r'(\{"\$(gte|lt|lte|gt|eq|ne|in)": \[[^\]]*\])(?=,|\])', r'\1}', query_string)
        try:
    ...
    def _has_invalid_top_level_operator(self, obj):
        ...
        but allow $and, $or, $nor at the top level."""
        allowed_top_level = {'$and', '$or', '$nor'}
        if isinstance(obj, dict):
            for k in obj.keys():
                if isinstance(k, str) and k.startswith('$') and k not in allowed_top_level:
                    return True
