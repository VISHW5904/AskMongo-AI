...
class EnhancedMongoDBBot:
    ...
    def _natural_language_to_query(self, user_question):
        # Pre-process any date, code or number matches from the question
...
7. Date Handling: If a specific date like "01/01/2025" is mentioned, filter for the entire day. For date ranges like "from 01/01/2025 to 10/01/2025", always interpret as January 1 to January 10 (not October). Use $gte for the start date and $lt for the day after the end date. Convert all date strings to ISODate format, e.g., ISODate("YYYY-MM-DDTHH:MM:SSZ").
...
23. IMPORTANT: Do NOT use a top-level $and in find queries. Instead, merge all conditions into a single dict. Example: find({"memberCode": "123", "dateTimeOfCollection": {"$gte": ...}}) NOT find({"$and": [...]})
Examples:
...
""" % (json.dumps(self.schema_info, indent=2), user_question)
        try:
            ...
            if isinstance(query_text, str):
                ...
                # Ensure proper closing of parentheses
                open_count = query_text.count('(')
                close_count = query_text.count(')')
                if open_count > close_count:
                    query_text += ')' * (open_count - close_count)
                # Remove any code block markers or language specifiers
                query_text = re.sub(r'```.*?\n', '', query_text)
                query_text = query_text.replace('```', '')
                # Find the actual query command
                lines = query_text.splitlines()
                for line in lines:
                    line = line.strip()
                    if any(line.startswith(cmd) for cmd in ['find(', 'aggregate(', 'distinct(', 'count_documents(']):
                        query_text = line
                        break
    ...
    def _merge_and_filter(self, query_dict):
        if isinstance(query_dict, dict) and "$and" in query_dict:
            merged = {}
    ...
    def _safe_eval(self, query_string):
        ...
        open_brackets = query_string.count('[')
        close_brackets = query_string.count(']')
        open_parens = query_string.count('(')
        close_parens = query_string.count(')')
        query_string += '}' * (open_braces - close_braces)
        query_string += ']' * (open_brackets - close_brackets)
        query_string += ')' * (open_parens - close_parens)
        ...
        try:
            ...
            print(f"_safe_eval parsing error: {e} on string: {query_string}")
            try:
                return eval(query_string, {"__builtins__": {}}, {"datetime": datetime, "True": True, "False": False, "None": None})
            except Exception as e_eval:
                print(f"_safe_eval eval error: {e_eval} on string: {query_string}")
                raise ValueError(f"Query parsing error with eval: {e_eval} on string: {query_string}\n\nThis may be due to mismatched brackets or malformed LLM output. Please try rephrasing your question or contact support.")
    def _fix_aggregation_pipeline(self, pipeline):
        if isinstance(pipeline, list) and pipeline:
            ...
            if "$match" in match_stage and "$expr" in match_stage["$match"]:
                ...
                if isinstance(expr, dict) and "$and" in expr and isinstance(expr["$and"], list):
                    new_and = []
                    for cond in expr["$and"]:
                        if isinstance(cond, dict):
                                ...
                                if op in cond:
                                    ...
                                    if not (isinstance(val, list) and len(val) == 2):
                                        if isinstance(val, dict):
                                            field = val.get("$field") or val.get("field")
                                            value = val.get("$value") or val.get("value")
                                            ...
                                            continue
                                    if not (isinstance(cond[op], list) and len(cond[op]) == 2):
                                        continue
                        new_and.append(cond)
                    expr["$and"] = new_and
        ...
        if isinstance(obj, dict):
            ...
            for item in obj:
                if self._has_invalid_top_level_operator(item):
                    return True
        return False
    def _validate_query_structure(self, query_type, parsed_query):
        if parsed_query is None:
            return "Query is empty or could not be parsed."
        if query_type == 'find':
            if not isinstance(parsed_query, dict):
                return "Find query must be a dictionary/object."
            for k in parsed_query.keys():
                if k in ['$and', '$or', '$nor', '$where']:
                    return f"Top-level operator '{k}' is not allowed in find queries."
                if k.startswith('$'):
                    return f"Top-level key '{k}' cannot be a MongoDB operator."
        elif query_type == 'aggregate':
            if not isinstance(parsed_query, list):
                return "Aggregation pipeline must be a list of stages."
            if not parsed_query:
                return "Aggregation pipeline is empty."
            first_stage = parsed_query[0]
            if not isinstance(first_stage, dict) or not any(k in first_stage for k in ['$match', '$group', '$project', '$sort', '$limit']):
                return "First stage of aggregation must be $match, $group, $project, $sort, or $limit."
        elif query_type == 'distinct':
            if parsed_query is not None and not isinstance(parsed_query, dict):
                return "Distinct filter must be a dictionary/object."
        elif query_type == 'count_documents':
            if parsed_query is not None and not isinstance(parsed_query, dict):
                return "Count_documents filter must be a dictionary/object."
        if isinstance(parsed_query, dict):
            if '{{' in str(parsed_query) or '}}' in str(parsed_query):
                return "Query contains double curly braces, which is invalid syntax."
            for k, v in parsed_query.items():
                if isinstance(v, dict) and '$in' in v and (not v['$in'] or not isinstance(v['$in'], list)):
                    return "Query contains an empty or invalid $in array."
        return None
    def _execute_mongodb_query(self, query_text):
        if query_text.startswith("error:"):
            return {"error": query_text}
        try:
            query_text = self._clean_query_string(query_text)
            if query_text.startswith('find('):
                ...
                filter_query = self._merge_and_filter(filter_query)
                validation_error = self._validate_query_structure('find', filter_query)
                if validation_error:
                    return {"error": f"Query validation failed: {validation_error}"}
                ...
                validation_error = self._validate_query_structure('aggregate', pipeline)
                if validation_error:
                    return {"error": f"Query validation failed: {validation_error}"}
                ...
                validation_error = self._validate_query_structure('distinct', filter_query)
                if validation_error:
                    return {"error": f"Query validation failed: {validation_error}"}
                if self._has_invalid_top_level_operator(filter_query):
                    return {"error": "Invalid query: top-level key cannot be a MongoDB operator like $gte, $lt, etc. Please rephrase your question."}
                ...
                filter_query = self._safe_eval(filter_str) if filter_str else {}
                validation_error = self._validate_query_structure('count_documents', filter_query)
                if validation_error:
                    return {"error": f"Query validation failed: {validation_error}"}
                if self._has_invalid_top_level_operator(filter_query):
                    return {"error": "Invalid query: top-level key cannot be a MongoDB operator like $gte, $lt, etc. Please rephrase your question."}
    ...
    def ask_question(self, user_question: str, sample_size: int = None):
        ...
        if sample_size is None:
            match = re.search(
                r'(?:list|show|display|find|any|top|first|last|unique|records|collections)?\s*(\d{1,3})\s*(?:unique|records|collections|dcs|members|results|entries)?',
        ...
        print(f" Raw LLM output: {mongo_query_str}")
        
        # Clean and fix the query string
        mongo_query_str = self._clean_query_string(mongo_query_str)
        print(f" Cleaned query string: {mongo_query_str}")
    ...
    def _clean_query_string(self, query_str):
        ...
        query_str = re.sub(r'```.*?\n|```', '', query_str)
        query_str = re.sub(r'^(python|json|mongodb)\n*', '', query_str, flags=re.IGNORECASE)
        
        # Ensure balanced parentheses
        open_count = query_str.count('(')
        close_count = query_str.count(')')
        if open_count > close_count:
            query_str += ')' * (open_count - close_count)
        
        # Enhanced: handle find(filter), find(filter, projection), find(filter, {"$limit": N}), find(filter, projection, {"$limit": N})
        find_match = re.match(r'find\((.*)\)$', query_str.strip())
        if find_match:
            args = find_match.group(1)
            ...
            # Always use the first dict as the filter
            if parts and parts[0].startswith('{'):
                query_str = f'find({parts[0]})'
        return query_str
